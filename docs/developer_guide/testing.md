...
When creating unit tests, please follow the same directory structure as the `src/` directory. In other words, create a new folder in the appropriate location for each server that unit tests are being written for. For example, the code for the `ChatDatabaseFactory` class has its source files at `mada/core/database/db_factory.py` and therefore its test files live at `unit/core/database/test_db_factory.py`.
